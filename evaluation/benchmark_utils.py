...
def pt_icp_refine(src_keypts, tgt_keypts, pred_trans):
    """
...
def icp_refine(src_keypts, tgt_keypts, src_normals, tgt_normals, pred_trans, inlier_thresh):
    """
    ICP algorithm to refine the initial transformation
    Input:
        - src_keypts [1, num_corr, 3] FloatTensor
    ...
    src_pcd = make_point_cloud(src_keypts.detach().cpu().numpy()[0])
    tgt_pcd = make_point_cloud(tgt_keypts.detach().cpu().numpy()[0])
    src_pcd.normals = o3d.utility.Vector3dVector(src_normals.detach().cpu().numpy()[0])
    tgt_pcd.normals = o3d.utility.Vector3dVector(tgt_normals.detach().cpu().numpy()[0])
    initial_trans = pred_trans[0].detach().cpu().numpy()
    # change the convension of transforamtion because open3d use left multi.
    # 95.38 k=0.25
    loss = o3d.pipelines.registration.TukeyLoss(k=0.25)
    refined_T = o3d.pipelines.registration.registration_icp(
        src_pcd, tgt_pcd, inlier_thresh, initial_trans,
        o3d.pipelines.registration.TransformationEstimationPointToPlane(loss)).transformation
    refined_T = torch.from_numpy(refined_T[None, :, :]).to(pred_trans.device).float()
    return refined_T
