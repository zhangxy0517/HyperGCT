...
def process_redwood(voxel_size=0.05):
    ...
    for scene in scene_list:
        scene_path = os.path.join("../data/redwood_simulated/", scene + '/fragments')
        pcd_list = os.listdir(scene_path)
...
def process_kitti(voxel_size=0.30, split='train'):
    ...
    for idx in tqdm(range(len(files)), ncols=50):
        ...
        filename = root + '/fcgf_icp/' + key + '.npy'
        ...
        pcd0 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(xyz_down0))
        ...
        pcd1 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(xyz_down1))
...
def process_lokitti(voxel_size=0.30):
    ...
    for idx in tqdm(range(len(files)), ncols=50):
        ...
        key = '%d_%d_%d' % (drive, t0, t1)
        filename = root + '/lo_fcgf_icp/' + key + '.npy'
        if key not in kitti_icp_cache:
            if not os.path.exists(filename):
                # work on the downsampled xyzs, 0.05m == 5cm
                _, sel0 = ME.utils.sparse_quantize(xyz0 / 0.05, return_index=True)
        ...
        filename = f"{root}/lo_fcgf_test/drive{drive}-pair{t0}_{t1}"
        np.savez_compressed(
            filename,
            xyz0=xyz_down0.astype(np.float32),
            xyz1=xyz_down1.astype(np.float32),
...
def process_kittilc(voxel_size=0.30, test_file=None):
    ...
    for seq, i, seq_db, j, tf_gt, overlap in tqdm(files, ncols=50):
        ...
        filename = f"{root}/lc_fcgf_{dis}/drive{seq}-pair{i}_{j}"
        np.savez_compressed(
            filename,
            xyz0=xyz_down0.astype(np.float32),
            xyz1=xyz_down1.astype(np.float32),
...
if __name__ == '__main__':
    ...
    #     process_kitti(voxel_size=0.30, split=split)

    #process_lokitti(voxel_size=0.30)
    process_kittilc(voxel_size=0.30, test_file='misc/kitti_lc/test_0_10.txt')
